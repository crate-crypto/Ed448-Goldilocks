...
mod tests {
    ...
    fn test_prepare_fixed_window() {
        ...
        let w = LookupTable::from(&p);
        assert_eq!(w.0.len(), TABLE_SIZE);

        let expected_window = [
            // 0
            ProjectiveNielsPoint {
                Y_minus_X: FieldElement::from_raw_slice([
                ...
                Y_plus_X: FieldElement::from_raw_slice([
                ...
                Td: FieldElement::from_raw_slice([
        ...
        for i in 0..TABLE_SIZE {
            ...
            assert!(expected_i.Z == w_i.Z);
            assert!(expected_i.Y_plus_X == w_i.Y_plus_X);
            assert!(expected_i.Y_minus_X == w_i.Y_minus_X);
            assert!(expected_i.Td == w_i.Td);
        }
    }
}
