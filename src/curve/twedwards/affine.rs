...
impl AffinePoint {
    ...
    fn add(&self, other: &AffinePoint) -> AffinePoint {
        ...
        let y_denominator = Fq::one() - EDWARDS_D * self.x * other.x * self.y * other.y;

        let x_numerator = self.x * other.y + self.y * other.x;
        let x_denominator = Fq::one() + EDWARDS_D * self.x * other.x * self.y * other.y;
